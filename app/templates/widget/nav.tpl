...
<div id="canvas_placeholder">
    {if $data == 'STATIC_RULE_AVALON'}
       <font>【规则】阿瓦隆规则 
                       ...
                       胜负判定：一局游戏一共设置5个任务，出现3个任务失败时，坏人直接胜利，出现3个任务成功时，刺客如杀对梅林，则坏人胜利，否则好人胜利。好坏兰斯
特的胜利条件根据游戏结束时自己的所属阵营来定。 

                       任务执行：所有玩家随机选择一个人作为初始的队长，队长根据5个任务规定的人数，选择相应数量的玩家组队，可以包含自己也可以不包含自己。选定队员
后，从队长的右手边开始逆时针发言，队长可以选择第一个发言，或者最后一个发言。发言仅一轮，完毕后，所有玩家投票选择同意组队还是反对组队，不可弃权，所有玩家同时亮出答案，如同意组队人数超过玩家总数的一半，则任务可以执行，等于或者小于一半时任务被延迟一次。 

                       延迟：任务被延迟后，不视为执行，可重新选择这个任务，但一局游戏任务只能出现4次延迟，此后队长无论如何选择队员，任务都必须执行。 

                       任务结果：任务执行时，所有队员从任务成功和失败中选择一个秘密给出，好人必须出成功，坏人可以选择出成功或失败，兰斯洛特*坏必须出失败。所有队
的结果将牌洗混后公布，根据其中的成功和失败牌的数量来判断任务成功或失败。 
                       ...
                       3、 派西维尔睁眼，梅林和莫甘娜举手 
                       天亮阶段： 
                       1、 随机（如非第一次游戏，则以上局最后一个队长的左手玩家为首任队长）选择一个玩家担任队长，队长从5个任务中选择一个，再选定足够人数的队员，
行发言和投票，决定任务执行或延迟。
